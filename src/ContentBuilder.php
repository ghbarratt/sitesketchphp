...
class ContentBuilder
{
	...
	public function setTemplate($template)
	{
		...
		if (!$template_found) {
			$this->errors[] = 'The template file: "' . $template . '" could not be found. Path is: ' . get_include_path();
		}
						...
						if ($cut_end !== false) {
							...
							if (is_array($value)) {
								...
								if ($is_array_numeric) {
									...
									$filled_template = $this->getTemplateWithReplacementsContent($embed_template, $value);
								}
